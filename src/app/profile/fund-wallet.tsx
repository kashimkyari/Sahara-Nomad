    ...
    return (
      <SafeAreaView style={styles.safeArea}>
        ...
        <View style={styles.successContainer}>
          <CheckCircle2 size={72} color={colors.secondary} strokeWidth={1.5} />
            ...
            {[
              ...
              <View key={r.label}>
                <View style={styles.receiptRow}>
                  ...
                  <Text style={[styles.receiptValue, r.label === 'Status' && { color: colors.secondary }]}>{r.value}</Text>
                </View>
              ...
              <View key={m.id}>
                <TouchableOpacity style={styles.methodRow} onPress={() => setMethod(m.id)}>
                  <View style={[styles.methodIconBox, method === m.id && styles.methodIconBoxActive]}>
                    {m.icon
                      ? <m.icon size={18} color={method === m.id ? colors.surface : colors.text} strokeWidth={2.5} />
                      : <Text style={[styles.ussdIcon, method === m.id && { color: colors.surface }]}>*#</Text>}
                  </View>
            ...
            <View style={styles.bankDetails}>
              ...
              {[
                ...
                <View key={u.bank} style={styles.bankDetailRow}>
                  ...
                  <Text style={[styles.bankDetailValue, { fontFamily: DT.typography.heading, color: colors.primary }]}>{u.code}</Text>
                </View>
              ))}
            </View>
          )}
...
const getStyles = (colors: any) => StyleSheet.create({
  ...
  nairaSign: { fontFamily: DT.typography.heading, fontSize: 32, color: colors.surface, marginRight: 6 },
  amountInput: {
    flex: 1, fontFamily: DT.typography.heading, fontSize: 42,
    color: colors.surface, padding: 0,
  },
  sectionLabel: {
  ...
  quickChipText: { fontFamily: DT.typography.bodySemiBold, fontSize: 14, color: colors.text },
  quickChipTextActive: { fontFamily: DT.typography.heading },
  methodGroup: {
    borderWidth: 2, borderColor: colors.text, backgroundColor: colors.surface, marginBottom: DT.spacing.md,
  ...
  ussdIcon: { fontFamily: DT.typography.heading, fontSize: 14, color: colors.text },
  methodInfo: { flex: 1 },
  methodLabel: { fontFamily: DT.typography.bodySemiBold, fontSize: 14, color: colors.text },
  methodSub: { fontFamily: DT.typography.body, fontSize: 11, color: colors.muted, marginTop: 1 },
  radio: {
    width: 20, height: 20, borderWidth: 2, borderColor: colors.text,
  ...
  bankDetails: {
    borderWidth: 2, borderColor: colors.secondary, backgroundColor: colors.surface,
    padding: DT.spacing.md, marginBottom: DT.spacing.lg,
  },
  bankDetailsTitle: {
    fontFamily: DT.typography.heading, fontSize: 12, color: colors.secondary,
    letterSpacing: 0.5, marginBottom: DT.spacing.md,
  },
  bankDetailRow: {
    flexDirection: 'row', justifyContent: 'space-between',
    paddingVertical: DT.spacing.sm, borderBottomWidth: 1, borderBottomColor: colors.background,
  },
  bankDetailLabel: { fontFamily: DT.typography.body, fontSize: 13, color: colors.muted },
  bankDetailValue: { fontFamily: DT.typography.bodySemiBold, fontSize: 13, color: colors.text },
  bankNote: {
  ...
  fundBtnDisabled: { backgroundColor: colors.muted, shadowOpacity: 0, elevation: 0 },
  fundBtnText: { fontFamily: DT.typography.heading, fontSize: 17, color: colors.surface },
  successContainer: { flex: 1, alignItems: 'center', paddingHorizontal: DT.spacing.lg, paddingTop: DT.spacing.xl, gap: DT.spacing.md },
  successAmount: { fontFamily: DT.typography.heading, fontSize: 40, color: colors.text },
  successTitle: { fontFamily: DT.typography.heading, fontSize: 24, color: colors.text },
  successSub: { fontFamily: DT.typography.body, fontSize: 14, color: colors.muted, textAlign: 'center' },
  receiptCard: {
    width: '100%', borderWidth: 2, borderColor: colors.text, backgroundColor: colors.surface,
    shadowColor: colors.text, shadowOffset: { width: 4, height: 4 }, shadowOpacity: 1, shadowRadius: 0, elevation: 5,
  },
  receiptRow: {
    flexDirection: 'row', justifyContent: 'space-between', padding: DT.spacing.md,
  },
  receiptLabel: { fontFamily: DT.typography.body, fontSize: 13, color: colors.muted },
  receiptValue: { fontFamily: DT.typography.bodySemiBold, fontSize: 13, color: colors.text },
  receiptDivider: { height: 2, backgroundColor: colors.text },
  doneBtn: {
